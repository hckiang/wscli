...
- ~reuse-text-message-strbuf~ - boolean. If ~nil~, each time before calling the user's handler function, a fresh
                                ...
                                will be overwritten after the handler exits. This option only affects
                                text messages but not ~:binary~, ~:ping~, and ~:pong~. Default is ~nil~.
- ~borrowed-bin-message-buf~ - boolean. Same as ~reuse-text-message-strbuf~ but for binary data, i.e.,
                               ~:binary~, ~:ping~, and ~:pong~. Again, this avoids allocation but the user
                               must understand that the byte array fed to their handler is only valid until
                               the handler exits. Defaults to ~nil~.
